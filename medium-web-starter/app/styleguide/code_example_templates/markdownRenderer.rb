  ...
  def block_code(code, language)
    ...
    if language and language.include?('example')
      if language.include?('js')
        ...
        '<div class="exampleContainer"><div class="codeExample">' + '<div class="exampleOutput">' + render_html(code, language) + '</div>' + '<button class="js-toggle-btn toggle-button toggle-button-styleguide" data-target="#' + random_number + '">&lt; /&gt;</button><div id="' + random_number + '" class="toggle-item toggle-item--styleguide"><div class="toggle-item__content"><div class="codeBlock"><div class="highlight"><pre>' + formatter.format(lexer.lex(code)) + '</pre></div></div></div></div></div></div>'
      end
    else
